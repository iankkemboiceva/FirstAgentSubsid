...
public class SchFeeAdapter extends RecyclerView.Adapter<SchFeeAdapter.MyViewHolder>   {
...
/*	public ImeiRvAdapter(List<ImeiList> planetList, Context ctx) {
    ...
    public SchFeeAdapter(Context context, List<ImeiList> planetList) {
    ...
    @Override
    public SchFeeAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
    ...
    @Override
    public void onBindViewHolder(SchFeeAdapter.MyViewHolder holder, int position) {
        ImeiList p = planetList.get(position);
        String dtype = p.getDtype();
        if(dtype == null){
            dtype = "N/A";
      ...
      holder.curr.setText(dtype);
        holder.dtype.setText(p.getImei());
