...
public static JSONArray getNubanAlgo(String account){
	...
	public static int getAccNoLength(Context ct){
		int rt = 0;
		try {
			SessionManagement session = new SessionManagement(ct);
			String sessac = session.getString("ACCLENGTH");
			rt =Integer.parseInt(sessac);
		}catch (Exception ex){
			ex.printStackTrace();
		}
		return rt;
	...
	public static String oldb64_sha256(String inputString) {
